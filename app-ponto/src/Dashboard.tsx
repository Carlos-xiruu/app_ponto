...
export default function Dashboard() {
  ...
  const buscarDados = async () => {
    ...
    data.forEach((ponto) => {
      ...
      if (agrupamento[chave].entrada && agrupamento[chave].saida) {
        ...
        if (horaEntradaDec <= 12.0 && horaSaidaDec >= 13.0) { minutos -= 60; agrupamento[chave].descontouAlmoco = true; }
  ...
  const fecharFolhaDoMes = async () => {
    if(!window.confirm(`ATENÇÃO GESTOR:\nVocê está prestes a FECHAR a folha de ${mesFiltro}.\nIsso enviará o espelho de ponto deste mês para todos os funcionários assinarem digitalmente pelo aplicativo.\n\nTem certeza que os registros estão corretos?`)) return;
    setCarregando(true);
    
    const inserts = funcionarios.map(f => ({
      funcionario_id: f.id,
      mes_ano: mesFiltro,
      status: 'pendente'
    }));
    ...
    else alert('Folha fechada com sucesso! Os colaboradores foram notificados.');
          ...
          <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 print:hidden">
            <div className="bg-[#0f172a] border border-slate-700 rounded-2xl w-full max-w-2xl p-8 shadow-2xl relative">
              ...
              <div className="flex flex-col items-center mb-6 border-b border-slate-800 pb-6">
                ...
                <h2 className="text-2xl font-bold font-['Montserrat'] text-white text-center">Auditoria de Assinatura Eletrônica</h2>
                <p className="text-sm text-slate-400">Laudo Técnico de Validade Jurídica (Lei 14.063/2020)</p>
              </div>

              <div className="space-y-4 mb-8">
                <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
                  <span className="block text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Assinado por</span>
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-bold text-slate-200">{certificadoSelecionado.nomeFuncionario}</span>
                    <span className="font-mono text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded border border-emerald-500/20">CPF: {certificadoSelecionado.cpfFuncionario}</span>
                  </div>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
                    <span className="block text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Competência (Folha)</span>
                    <span className="font-bold text-slate-200">{certificadoSelecionado.folha.mes_ano.split('-')[1]}/{certificadoSelecionado.folha.mes_ano.split('-')[0]}</span>
                  </div>
                  <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
                    <span className="block text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Data/Hora da Assinatura</span>
                    <span className="font-bold text-slate-200">{new Date(certificadoSelecionado.folha.data_assinatura).toLocaleString('pt-BR')}</span>
                  </div>
                </div>

                <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800 space-y-2">
                  <div>
                    <span className="block text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-0.5">Rastreabilidade de Rede (IP)</span>
                    <span className="font-mono text-xs text-blue-400">{certificadoSelecionado.folha.ip_assinatura || 'Não registrado'}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-0.5 mt-2">Coordenada GPS no momento do aceite</span>
                    <span className="font-mono text-xs text-blue-400">{certificadoSelecionado.folha.gps_assinatura || 'Não registrado'}</span>
                  </div>
                </div>

                <div className="bg-[#020617] p-4 rounded-xl border border-slate-700">
                  ...
                  <span className="font-mono text-[10px] text-slate-300 break-all">{certificadoSelecionado.folha.hash_auditoria}</span>
                </div>
              </div>

              <div className="flex gap-4">
                <button onClick={() => setCertificadoSelecionado(null)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white font-bold py-4 rounded-xl transition-colors">Voltar</button>
                <button onClick={() => window.print()} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-blue-900/20 flex justify-center items-center gap-2"><FileText size={18}/> Imprimir Laudo</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-[#0f172a] border border-slate-700 rounded-3xl w-full max-w-4xl p-6 md:p-8 shadow-2xl relative flex flex-col md:flex-row gap-8 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={salvarNovaObra} className="flex flex-col gap-4">
                  ...
                  <div className="relative">
                    ...
                    <div className="flex gap-2 relative">
                      <input type="text" value={formObra.buscaEndereco || ''} onChange={e => { setFormObra({...formObra, buscaEndereco: e.target.value}); if (e.target.value === '') setResultadosBusca([]); }} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); buscarCoordenadasPorEndereco(); } }} className="flex-1 bg-[#020617] border border-slate-700 rounded-xl p-3.5 text-sm text-white focus:border-blue-500 outline-none transition-colors" placeholder="Ex: Avenida Paulista, 1000, São Paulo" />
                      <button type="button" onClick={buscarCoordenadasPorEndereco} disabled={buscandoEndereco || !formObra.buscaEndereco} className="bg-blue-600 hover:bg-blue-500 text-white px-5 rounded-xl transition-all shadow-md disabled:opacity-50 flex items-center justify-center shrink-0">
                  ...
                  <div className="flex items-center gap-3 my-1 opacity-30"><div className="h-[1px] bg-slate-500 flex-1"></div><div className="w-1.5 h-1.5 rounded-full bg-slate-500"></div><div className="h-[1px] bg-slate-500 flex-1"></div></div>
                  <button type="button" onClick={pegarLocalizacaoParaObra} className="w-full flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-200 py-3.5 rounded-xl text-sm font-semibold transition-all shadow-sm">
                  ...
                  <div className="mt-2">
                    ...
                    <input type="text" required value={formObra.gps} onChange={e => setFormObra({...formObra, gps: e.target.value})} className="w-full bg-[#020617]/50 border border-slate-800 rounded-xl p-3.5 text-sm text-blue-400 font-mono outline-none transition-colors" placeholder="-26.4842,-49.0984" />
                  </div>
                  <button type="submit" disabled={carregando} className="mt-2 w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-emerald-900/20">Cadastrar e Ativar Obra</button>
                </form>
              </div>
              <div className="flex-1 flex flex-col mt-4 md:mt-0">
                ...
                <div className="flex flex-col gap-3 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                  {obrasList.length === 0 ? (
                    <div className="text-center p-8 bg-[#020617] border border-slate-800 rounded-2xl"><Building2 size={32} className="text-slate-600 mx-auto mb-3" /><span className="text-sm text-slate-400">O seu banco de obras está vazio. Adicione a primeira!</span></div>
                  ) : (
                    obrasList.map(obra => (
                      <div key={obra.id} className="bg-slate-900/80 hover:bg-slate-800/80 border border-slate-700/60 rounded-xl p-4 flex justify-between items-center transition-colors group">
                        <div className="overflow-hidden">
                          ...
                          <span className="block text-[11px] text-slate-400 font-mono mt-1 flex items-center gap-1"><MapPin size={10} className="text-blue-500" /> {obra.localizacao_gps}</span>
                        </div>
                        <button type="button" onClick={() => apagarObra(obra.id)} className="p-2.5 bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-white rounded-lg transition-colors ml-3 shrink-0" title="Excluir Obra"><Trash2 size={16} /></button>
                      </div>
                    ))
                  )}
                </div>
        ...
        <div className="block">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-5">
            <div>
              ...
              <p className="text-slate-400 text-sm">Gestão de horas, equipe e auditoria de assinaturas.</p>
            </div>
            
            <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 w-full md:w-auto">
              {/* O BOTÃO QUE ESTAVA FALTANDO */}
              <button onClick={fecharFolhaDoMes} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm rounded-xl transition-all shadow-lg shadow-blue-900/30">
                <FileSignature size={18} /> Fechar Mês {mesFiltro.split('-')[1]}/{mesFiltro.split('-')[0]}
              </button>
              ...
              <button onClick={() => setModalObraAberto(true)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-3.5 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700/60 font-semibold text-sm rounded-xl transition-all shadow-sm">
                <Building2 size={18} /> Obras
              </button>

              <button onClick={() => { setExtratoSelecionado(null); setTimeout(() => window.print(), 100); }} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-3.5 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700/60 font-semibold text-sm rounded-xl transition-all shadow-sm">
                <FileText size={18} /> PDF Geral
              </button>

              <button onClick={enviarRelatorioGeralWhats} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-3.5 bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-400 font-semibold text-sm rounded-xl transition-all shadow-sm">
                <Share2 size={18} /> Whats Geral
              </button>
            </div>
          </div>

          <div className="mb-8">
            <div className="bg-[#0f172a] border border-slate-700 rounded-2xl p-5 md:w-1/3 shadow-xl mb-6">
              <label className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-3 block">Filtro de Competência (Mês/Ano)</label>
            ...
            <div className="bg-[#0f172a]/80 border border-slate-800 rounded-2xl shadow-xl overflow-hidden mb-8">
              <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                  ...
                  Controle de Folha e Assinaturas
                </h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead>
                    <tr className="bg-slate-900/80 border-b border-slate-800">
                      <th className="p-4 text-slate-400 text-xs font-semibold uppercase tracking-wider">Colaborador</th>
                      <th className="p-4 text-slate-400 text-xs font-semibold uppercase tracking-wider text-center">Horas no Mês</th>
                      <th className="p-4 text-slate-400 text-xs font-semibold uppercase tracking-wider text-center">Status da Folha</th>
                      <th className="p-4 text-slate-400 text-xs font-semibold uppercase tracking-wider text-right">Ações</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/40">
                    {funcionarios.length === 0 ? (
                       <tr><td colSpan="4" className="p-8 text-center text-slate-500">Nenhum funcionário cadastrado no sistema ainda.</td></tr>
                    ) : (
                      funcionarios.map(func => {
                        ...
                        if (folhaDB) {
                          if (folhaDB.status === 'pendente') {
                            badgeStatus = <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/20"><AlertTriangle size={12} /> Aguardando Assinatura</span>;
                          } else if (folhaDB.status === 'assinado') {
                            badgeStatus = <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><Fingerprint size={12} /> Assinado Digitalmente</span>;
                          }
                        }
                        ...
                        return (
                          <tr key={func.id} className="hover:bg-slate-800/30 transition-colors">
                            <td className="p-4">
                              <span className="font-bold text-slate-200 block">{func.nome}</span>
                              <span className={`text-[10px] font-mono mt-1 block ${func.cpf ? 'text-slate-400' : 'text-red-400 font-bold'}`}>
                                {func.cpf ? `CPF: ${func.cpf}` : 'SEM CPF CADASTRADO NO APP'}
                              </span>
                            </td>
                            ...
                            <td className="p-4 text-right">
                              <div className="flex justify-end gap-2">
                                {resumo && (
                                  <button onClick={() => setExtratoSelecionado(resumo)} title="Ver Espelho Mensal" className="p-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg transition-colors border border-slate-700">
                                    <Eye size={16} />
                                  </button>
                                )}
                                {folhaDB?.status === 'assinado' && (
                                  <button onClick={() => setCertificadoSelecionado({ folha: folhaDB, nomeFuncionario: func.nome, cpfFuncionario: func.cpf })} title="Ver Certificado de Assinatura" className="px-3 py-2 bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-400 border border-emerald-500/30 text-xs font-bold uppercase tracking-wider rounded-lg transition-colors flex items-center gap-2">
          ...
          {/* TABELA DETALHADA DE PONTOS DIÁRIOS */}
          <div className="bg-[#0f172a]/60 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/30">
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Espelho de Ponto Geral Diário</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-slate-900/70 border-b border-slate-800">
                    ...
                    <th className="p-5 text-slate-400 text-xs font-semibold uppercase tracking-wider">Jornada Diária</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/40">
        ...
        {certificadoSelecionado ? (
          <div className="certificado-container">
            ...
            <div style={{ marginTop: '40px' }}>
              <p style={{ fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '5px' }}>Chave Criptográfica de Imutabilidade (Hash SHA-256):</p>
              <div className="certificado-hash">{certificadoSelecionado.folha.hash_auditoria}</div>
            </div>
          </div>
        ) : extratoSelecionado ? (
          <div>
            ...
            <table className="pdf-table" style={{ marginTop: '5px', marginBottom: '20px' }}>
              <thead>
                <tr><th style={{ width: '50%' }}>Colaborador</th><th style={{ width: '50%' }}>Função / Cargo</th></tr>
              </thead>
              <tbody>
                <tr><td style={{ fontWeight: 'bold', fontSize: '12px' }}>{extratoSelecionado.nome}</td><td style={{ fontWeight: 'bold', fontSize: '12px' }}>{extratoSelecionado.cargo}</td></tr>
              </tbody>
            </table>
            <div className="pdf-section">2. Espelho de Ponto Detalhado</div>
            <table className="pdf-table" style={{ marginTop: '5px' }}>
              <thead>
                <tr><th style={{ width: '15%' }}>Data</th><th style={{ width: '35%' }}>Obra Local</th><th style={{ width: '15%' }}>Entrada</th><th style={{ width: '15%' }}>Saída</th><th style={{ width: '20%', textAlign: 'right' }}>Total Diário</th></tr>
              </thead>
              <tbody>
                {extratoSelecionado.logs.map((l, i) => (
                  <tr key={i}>
          ...
          <div>
            ...
            <table className="pdf-table" style={{ marginTop: '5px', marginBottom: '20px' }}>
              <thead>
                <tr><th style={{ width: '50%' }}>Nome do Colaborador</th><th style={{ width: '30%' }}>Função Registrada</th><th style={{ width: '20%', textAlign: 'right' }}>Carga Horária Total</th></tr>
              </thead>
              <tbody>
                {resumoMensal.map((r, i) => (
                  <tr key={i}><td style={{ fontWeight: 'bold' }}>{r.nome}</td><td>{r.cargo}</td><td style={{ textAlign: 'right', fontFamily: 'monospace', fontWeight: 'bold', fontSize: '12px' }}>{r.horasFormatadas}</td></tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="pdf-table" style={{ marginTop: '5px' }}>
              <thead>
                <tr><th style={{ width: '25%' }}>Colaborador / Obra</th><th style={{ width: '15%' }}>Data</th><th style={{ width: '15%' }}>Entrada</th><th style={{ width: '25%' }}>Saída</th><th style={{ width: '20%', textAlign: 'right' }}>Total Dia</th></tr>
              </thead>
              <tbody>
                {pontosAgrupados.map((linha, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ textAlign: 'right', fontFamily: 'monospace', fontWeight: 'bold' }}>{linha.minutosTrabalhadosDia > 0 ? `${Math.floor(linha.minutosTrabalhadosDia / 60)}h ${(linha.minutosTrabalhadosDia % 60).toString().padStart(2, '0')}m` : '-'}{linha.descontouAlmoco ? ' (Desc.)' : ''}</td>
                  </tr>
                ))}
              </tbody>
            </table>
