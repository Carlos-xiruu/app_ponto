...
export default function BaterPonto() {
  ...
  const carregarDadosDoFuncionario = async () => {
    ...
    const { data: historico } = await supabase.from('registros_ponto').select('tipo_registro, data_hora, localizacao_gps, obra_nome').eq('funcionario_id', user.id).order('data_hora', { ascending: false }).limit(2);
  ...
  const buscarFolhasPendentes = async (userId) => {
    const { data } = await supabase.from('folhas_pagamento').select('*').eq('funcionario_id', userId).eq('status', 'pendente');
  ...
  const abrirModalAssinatura = async (folha) => {
    ...
    const { data } = await supabase.from('registros_ponto').select('tipo_registro, data_hora, obra_nome').eq('funcionario_id', perfil.id).gte('data_hora', dataInicio).lte('data_hora', dataFim).order('data_hora', { ascending: true });
    ...
    if (data) {
      ...
      setEspelhoAssinatura({
        dias: Object.values(dias),
        horasFormatadas: `${Math.floor(totalMinutosMes / 60)}h ${(totalMinutosMes % 60).toString().padStart(2, '0')}m`
      });
    }
  };
  ...
  const assinarDocumento = async () => {
    if (!perfil.cpf) { alert("Atenção: Você precisa de um CPF cadastrado para assinar. Peça ao RH."); return; }
    if (!concordoTermos) { alert("Você precisa marcar a caixa confirmando a veracidade das informações."); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      async (posicao) => {
        ...
        const { error } = await supabase.from('folhas_pagamento').update({
            status: 'assinado', data_assinatura: dataExata, ip_assinatura: ip, gps_assinatura: gpsAtual, hash_auditoria: hash
        }).eq('id', folhaParaAssinar.id);

        if (error) { mostrarAviso('Falha na comunicação com o cartório digital.'); } 
        else {
      ...
      () => { mostrarAviso('Você precisa permitir o GPS para ter validade jurídica.'); setAssinando(false); },
  ...
  const salvarPonto = async (foto, local, tipo, nomeObra) => {
    const { error } = await supabase.from('registros_ponto').insert({ funcionario_id: perfil.id, tipo_registro: tipo, data_hora: new Date().toISOString(), foto_url: foto, localizacao_gps: local, obra_nome: nomeObra });
    if (error) mostrarAviso('Falha de Rede: Tente de novo.'); else { mostrarAviso('Ponto Salvo com Sucesso!'); carregarDadosDoFuncionario(); }
    setCarregando(false);
  };
  ...
  const sairApp = async () => { await supabase.auth.signOut(); window.location.reload(); };
    ...
    <div className="h-screen bg-[#020617] font-['Inter'] text-slate-100 flex flex-col overflow-hidden relative">
      ...
      <div className="w-full bg-[#0f172a]/90 backdrop-blur-md border-b border-slate-800 p-4 shrink-0 flex justify-between items-center z-20">
        <div className="flex items-center gap-3">
          {perfil.avatar_url ? ( <img src={perfil.avatar_url} alt="Avatar" className="w-10 h-10 rounded-full object-cover border border-slate-600" /> ) : ( <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-emerald-500 font-bold border border-slate-700">{perfil.nome ? perfil.nome.charAt(0).toUpperCase() : <User size={20} />}</div> )}
          <div className="flex flex-col"><span className="font-['Montserrat'] font-bold text-sm text-slate-100 leading-tight">Olá, {perfil.nome || 'Carregando...'}</span><span className="text-xs text-slate-400 font-medium flex items-center gap-1"><Briefcase size={10} /> {perfil.funcao || 'Função não definida'}</span></div>
        </div>
        <div className="flex items-center gap-2">
          <div className="text-emerald-500 hidden sm:block mr-2"><CheckCircle2 size={24} /></div>
          <button onClick={sairApp} className="p-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors border border-transparent hover:border-red-500/20" title="Sair"><LogOut size={20} /></button>
        </div>
      </div>

      {/* ALERTA GLOBAL DE PENDÊNCIA JURÍDICA */}
      {folhasPendentes.length > 0 && !modalAssinaturaAberto && (
        <div className="w-full bg-red-950/90 border-b border-red-900/50 p-3 shrink-0 flex items-center justify-between z-10 animate-in slide-in-from-top-2">
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 bg-red-500/20 rounded-lg"><ShieldAlert size={16} className="text-red-400" /></div>
            <div className="flex flex-col"><span className="text-xs font-bold text-red-200">Ação Necessária</span><span className="text-[10px] text-red-300">Sua folha de ponto está pronta para assinatura.</span></div>
          </div>
          <button onClick={() => abrirModalAssinatura(folhasPendentes[0])} className="px-3 py-1.5 bg-red-500 hover:bg-red-400 text-white text-[10px] font-bold uppercase tracking-wider rounded border border-red-400/50 shadow-sm transition-colors">Conferir</button>
        </div>
      )}
                ...
                {dropdownAberto && (
                  <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-[#0f172a] border border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-40 animate-in fade-in slide-in-from-top-2">
                    <ul className="max-h-60 overflow-y-auto py-2 divide-y divide-slate-800/50 custom-scrollbar">
                      <li onClick={() => { setObraSelecionadaId(''); setDropdownAberto(false); }} className="px-4 py-3 text-sm text-slate-400 hover:bg-slate-800 cursor-pointer transition-colors">Nenhuma / Limpar seleção</li>
                      {obrasList.map(obra => ( <li key={obra.id} onClick={() => { setObraSelecionadaId(obra.id); setDropdownAberto(false); }} className={`px-4 py-3 text-sm cursor-pointer transition-colors flex items-center justify-between ${obraSelecionadaId === obra.id ? 'bg-blue-600/10 text-blue-400 font-bold' : 'text-slate-200 hover:bg-slate-800'}`}><span className="truncate">{obra.nome}</span>{obraSelecionadaId === obra.id && <CheckCircle2 size={16} className="shrink-0" />}</li> ))}
                    </ul>
                  </div>
                )}
              </div>
            ...
            <div className="bg-[#0f172a] border border-slate-700 rounded-3xl p-6 shadow-2xl">
              <form onSubmit={salvarPerfil} className="flex flex-col gap-6">
                <div className="flex flex-col items-center gap-3">
                  <div className="relative group cursor-pointer mt-2">{perfil.avatar_url ? ( <img src={perfil.avatar_url} alt="Sua Foto" className="w-28 h-28 rounded-full object-cover border-4 border-slate-800 shadow-xl" /> ) : ( <div className="w-28 h-28 rounded-full bg-slate-800 border-2 border-dashed border-slate-600 flex items-center justify-center text-slate-500"><User size={40} /></div> )}<label className="absolute inset-0 flex items-center justify-center bg-black/60 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">{fazendoUpload ? <Loader2 size={24} className="text-white animate-spin" /> : <Upload size={24} className="text-white" />}<input type="file" accept="image/*" onChange={handleUploadFoto} disabled={fazendoUpload} className="hidden" /></label></div>
                  <span className="text-xs text-slate-400">Toque na foto para alterar</span>
                </div>
                <div><label className="text-xs text-slate-400 mb-1 block font-semibold uppercase tracking-wider">Sua Função</label><input type="text" value={perfil.funcao || ''} onChange={e => setPerfil({...perfil, funcao: e.target.value})} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3.5 text-sm text-white focus:border-emerald-500 outline-none transition-all" /></div>
                <button type="submit" disabled={carregando || fazendoUpload} className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl transition-colors shadow-lg"><Save size={18} /> Salvar Alterações</button>
              </form>
            </div>
            <button onClick={sairApp} className="mt-8 w-full flex items-center justify-center gap-2 text-red-400 font-bold py-4 rounded-xl border border-red-900/30 hover:bg-red-500/10 transition-colors"><LogOut size={18} /> Sair do Aplicativo</button>
          </div>
        )}
      </div>
      ...
      {status && (
        <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 w-[90%] max-w-sm flex items-start text-left gap-3 p-4 rounded-2xl text-sm font-medium border backdrop-blur-xl z-50 shadow-2xl animate-in slide-in-from-bottom-2 ${status.includes('Sucesso') ? 'bg-emerald-950/90 border-emerald-500/50 text-emerald-100' : 'bg-red-950/90 border-red-500/50 text-red-100'}`}>
          <div className="mt-0.5 shrink-0">{status.includes('Sucesso') ? <CheckCircle2 size={20} className="text-emerald-400" /> : <AlertCircle size={20} className="text-red-400" />}</div> <span className="leading-snug">{status}</span>
        </div>
      )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#0b1120]/95 backdrop-blur-lg border-t border-slate-800 shrink-0 z-40 px-6 pb-safe">
        <div className="flex justify-between items-center max-w-sm mx-auto h-20">
          <button onClick={() => setAbaAtiva('inicio')} className={`flex flex-col items-center gap-1.5 w-20 transition-colors ${abaAtiva === 'inicio' ? 'text-emerald-400' : 'text-slate-500 hover:text-slate-300'}`}><Home size={24} className={abaAtiva === 'inicio' ? 'fill-emerald-400/20' : ''} /><span className="text-[10px] font-bold uppercase tracking-wider">Início</span></button>
          <button onClick={() => setAbaAtiva('registros')} className={`flex flex-col items-center gap-1.5 w-20 transition-colors ${abaAtiva === 'registros' ? 'text-emerald-400' : 'text-slate-500 hover:text-slate-300'}`}><ClipboardList size={24} /><span className="text-[10px] font-bold uppercase tracking-wider">Registros</span></button>
          <button onClick={() => setAbaAtiva('perfil')} className={`flex flex-col items-center gap-1.5 w-20 transition-colors ${abaAtiva === 'perfil' ? 'text-emerald-400' : 'text-slate-500 hover:text-slate-300'}`}><User size={24} className={abaAtiva === 'perfil' ? 'fill-emerald-400/20' : ''} /><span className="text-[10px] font-bold uppercase tracking-wider">Perfil</span></button>
        </div>
      </div>
          ...
          <div className="bg-[#0f172a] w-full h-full sm:h-auto sm:max-h-[90vh] sm:rounded-3xl shadow-2xl flex flex-col relative animate-in zoom-in-95">
            <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-[#0b1120] shrink-0 sm:rounded-t-3xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold font-['Montserrat'] text-white leading-tight">Assinatura Digital</h2>
                  <p className="text-xs text-slate-400">Competência: {folhaParaAssinar.mes_ano.split('-')[1]}/{folhaParaAssinar.mes_ano.split('-')[0]}</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
              ...
              {!espelhoAssinatura ? (
                <div className="flex flex-col items-center justify-center py-10"><Loader2 className="animate-spin text-emerald-500 mb-3" size={32} /><span className="text-sm text-slate-400">Processando seu espelho...</span></div>
              ) : (
                <>
                  <div className="w-full bg-emerald-950/30 border border-emerald-900/50 rounded-2xl p-5 mb-6 flex flex-col items-center">
                    <span className="text-xs text-emerald-400 font-bold uppercase tracking-wider mb-1">Total Apurado</span>
            ...
            <div className="p-5 border-t border-slate-800 bg-[#0b1120] shrink-0 sm:rounded-b-3xl">
              ...
              >
                <div className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 mt-0.5 ${concordoTermos ? 'bg-emerald-500 border-emerald-500 text-[#020617]' : 'border-slate-600 bg-slate-800'}`}>
                  {concordoTermos && <CheckCircle2 size={14} />}
                </div>
                <label className="text-[11px] text-slate-300 leading-snug cursor-pointer">
                  Declaro que li e conferi as informações acima, que são verdadeiras. Autorizo a captura do meu IP e coordenada GPS atual para fins de validação jurídica (Lei 14.063/2020).
                </label>
              </div>
              
              <button 
                onClick={assinarDocumento} 
                disabled={assinando || !espelhoAssinatura || !concordoTermos} 
